const rawEvents = window.APP_CONFIG.rawEvents || [];
    const currentUserId = window.APP_CONFIG.currentUserId;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // [BARU] Sama seperti admin/schedule.js: #formContent memakai
        // "overflow-hidden" dan #scheduleForm memakai "overflow-y-auto",
        // sehingga bubble validasi bawaan browser (mis. dropdown "Dosen
        // Pengampu"/"Ruangan" yang required tapi belum dipilih) menjadi
        // tidak terlihat dan form gagal submit tanpa pemberitahuan apapun.
        // Tangani validasi secara manual di sini.
        ...
        if (scheduleFormEl) {
            scheduleFormEl.addEventListener('submit', function(e) {
                if (!this.checkValidity()) {
                    ...
                    showCustomAlert('error', 'Form Belum Lengkap', 'Mohon lengkapi semua field yang wajib diisi (ditandai dengan garis merah).');
                }
            });
        }
    ...
    function openFormModal(mode, eventData = null) {
        ...
        if (mode === 'add') {
            ...
            document.getElementById('scheduleForm').action = window.APP_CONFIG.baseUrl + '/user/addSchedule';
            document.getElementById('inputDate').value = selectedDateStr;
            // [BARU - Modul Dosen] pastikan input "Tambah Dosen Baru" tertutup
            // & tidak wajib saat membuka form Tambah.
            ...
            document.getElementById('scheduleForm').action = window.APP_CONFIG.baseUrl + '/user/editSchedule';
            ...
            // input "Tambah Dosen Baru" disembunyikan & dikosongkan setiap
            // kali modal edit dibuka.
            document.getElementById('inputIdDosen').value = props.idDosen || '';
            if (document.getElementById('inputDosenBaru')) {
                document.getElementById('inputDosenBaru').classList.add('hidden');
                document.getElementById('inputDosenBaru').required = false;
    ...
    function triggerDelete(id, type) {
        deleteUrl = `${window.APP_CONFIG.baseUrl}/user/deleteSchedule?id=${id}&type=${type}`;
