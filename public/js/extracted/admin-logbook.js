    ...
    function renderTable(logs) {
        ...
        logs.forEach(log => {
            ...
            if(log.proof_in) {
                ...
                proofBtn = `<button onclick="viewEvidence('Bukti', '<?= BASE_URL ?>/uploads/${folder}/${log.proof_in}')" class="px-2 py-1 bg-blue-50 text-blue-600 border border-blue-200 rounded text-[10px] font-bold hover:bg-blue-100 transition"><i class="fas fa-image text-[10px] mr-1"></i>Lihat</button>`;
            }
            ...
            if(log.status == 'Hadir') {
                if(log.proof_out) {
                    proofOutBtn = `<button onclick="viewEvidence('Bukti Pulang', '<?= BASE_URL ?>/uploads/attendance/${log.proof_out}')" class="px-2 py-1 bg-orange-50 text-orange-600 border border-orange-200 rounded text-[10px] font-bold hover:bg-orange-100 transition"><i class="fas fa-image text-[10px] mr-1"></i>Lihat</button>`;
                } else {
            ...
            if(['Izin', 'Sakit'].includes(log.status) && log.proof_izin) {
                proofIzinBtn = `<button onclick="viewEvidence('Bukti ${log.status}', '<?= BASE_URL ?>/uploads/leaves/${log.proof_izin}')" class="px-2 py-1 bg-orange-50 text-orange-600 border border-orange-200 rounded text-[10px] font-bold hover:bg-orange-100 transition"><i class="fas fa-file-pdf text-[10px] mr-1"></i>Lihat</button>`;
            }
    ...
    document.getElementById('confirmResetBtn').addEventListener('click', function() {
        ...
        fetch('<?= BASE_URL ?>/admin/reset_logbook', { 
            method: 'POST', 
            body: fd 
        })
        .then(res => {
