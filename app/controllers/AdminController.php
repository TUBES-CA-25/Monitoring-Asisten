...
class AdminController extends Controller {
    ...
    public function editUser() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $userModel = $this->model('UserModel');
            $oldUser = $userModel->getUserById($_POST['id_user']);
            ...
            if (!empty($_POST['password'])) {
                $userModel->changePassword($data['id'], $_POST['password']);
            }

            if ($userModel->updateUser($data)) {
                echo json_encode(['status' => 'success', 'title' => 'Berhasil', 'message' => 'Data pengguna berhasil disimpan.']);
            } else {
                echo json_encode(['status' => 'error', 'title' => 'Gagal', 'message' => 'Terjadi kesalahan sistem.']);
            }
            exit;
        }
    }
    ...
    public function addSchedule() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $userId = ($type == 'umum') ? $_SESSION['profil_id'] : ($_POST['user_id'] ?? null);
            ...
            if ($this->model('ScheduleModel')->createSchedule($data)) { 
                $_SESSION['flash'] = ['type' => 'success', 'title' => 'Sukses', 'message' => 'Jadwal berhasil dibuat.']; 
            } else { 
    ...
    public function deleteSchedule() {
        ...
        if (isset($_GET['id']) && isset($_GET['type'])) {
            if ($this->model('ScheduleModel')->deleteSchedule($_GET['id'], $_GET['type'], $_SESSION['profil_id'])) {
    ...
    public function reset_logbook() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $idRef = $_POST['id_ref']; $type = $_POST['type']; $mode = $_POST['mode'];
    ...
    public function updateProfile() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $isCompleted = (!empty($_POST['name']) && !empty($_POST['phone'])) ? 1 : 0;
    ...
    public function assistantDetail($id) {
        ...
        $this->view('layout/header', $data);
        $this->view('layout/sidebar', $data);
        $this->view('admin/detail_assistant', $data);
        $this->view('layout/footer');
    }
}
?>