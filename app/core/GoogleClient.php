...
class GoogleClient {
    ...
    public function __construct() {
        ...
        // sudah dimuat secara global lewat app/config/config.php (dibaca dari .env).
    ...
    /**
     * Cek apakah kredensial Google OAuth sudah diisi di .env.
     * Dipakai oleh Controller/View agar tidak menampilkan tombol
     * "Hubungkan Google Calendar" jika integrasi belum dikonfigurasi.
     */
    ...
    public function getValidAccessToken($userId) {
        // Jika kredensial Google OAuth belum dikonfigurasi di .env,
        // jangan lakukan apa pun (sinkronisasi dilewati secara aman).
    ...
    private function makeRequest($url, $params, $method = 'GET', $accessToken = null) {
        ...
        // Jika request gagal di level jaringan (mis. timeout/tidak ada koneksi),
        // jangan biarkan kode lain (ScheduleModel) crash - cukup log & return null.
        if ($result === false) {
            if (APP_DEBUG) {
                error_log('[GoogleClient] cURL error saat memanggil ' . $url . ': ' . $curlError);
            }
            return null;
        }
        ...
        // Return true untuk DELETE 204 No Content (Success)
        if ($method == 'DELETE' && $httpCode == 204) return true;

        // Log response error dari Google API (mis. token invalid, scope kurang)
        // hanya saat APP_DEBUG aktif, agar mudah didiagnosa tanpa membocorkan
        // detail ke response API yang dipakai aplikasi mobile.
        if (APP_DEBUG && $httpCode >= 400) {
            error_log('[GoogleClient] HTTP ' . $httpCode . ' dari ' . $url . ': ' . $result);
        }
