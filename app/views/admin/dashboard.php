<div class="max-w-7xl mx-auto space-y-8 animate-enter">
            ...
            <div class="carousel-container">
                <div class="carousel-track">
                    ...
                    foreach($allCards as $asisten): 
                        ...
                        // [BARU] Sertakan id_hash agar JS tidak perlu mengekspos id integer di URL
                        $asisten['id_hash'] = HashHelper::encode((int)($asisten['id'] ?? $asisten['id_user'] ?? 0));
                    ...
                    <div class="polaroid-card <?= $cardExtra ?> rounded-xl cursor-pointer group relative hover:-translate-y-1 transition-all duration-300" onclick="openDetailModal(<?= $jsonUser ?>)">
                        ...
                        <div class="aspect-square bg-gray-100 mb-3 border border-gray-100 overflow-hidden rounded-lg relative">
                            <img src="<?= $photoPath ?>"
                                ...
                                alt="<?= htmlspecialchars($asisten['name'], ENT_QUOTES, 'UTF-8') ?>">
                            <?php
                                $timingBadge = $asisten['timing_badge'] ?? null;
                                if ($timingBadge === 'pulang_cepat'): ?>
                            <div class="attendance-stamp stamp-early">
                                <span class="stamp-label">PULANG<br>CEPAT</span>
                            </div>
                            <?php elseif ($timingBadge === 'terlambat'): ?>
                            <div class="attendance-stamp stamp-late">
                                <span class="stamp-label">TERLAMBAT</span>
                            </div>
                            <?php elseif ($timingBadge === 'tepat_waktu'): ?>
                            <div class="attendance-stamp stamp-ontime">
                                <span class="stamp-label">TEPAT<br>WAKTU</span>
                            </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-[10px] text-gray-400 font-bold uppercase mt-1"><?= htmlspecialchars($asisten['jabatan'] ?? 'Asisten', ENT_QUOTES, 'UTF-8') ?></p>
                        </div>
    ...
    <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        ...
        <p class="text-xs text-gray-400 mb-3">Bandingkan performa antar asisten (seluruh lab) berdasarkan metrik yang dipilih.</p>
        ...
        <div class="w-full md:w-1/3 bg-gray-50 border-r border-gray-100 flex flex-col">
            <div class="p-6 flex flex-col items-center text-center overflow-y-auto custom-scrollbar h-full">
                ...
                <div class="mt-6 w-full space-y-2">
                    <!-- [BARU – Tahap 30] Tombol Jadwal Lengkap (dikunci jika nonaktif) -->
                    <a id="btnSchedule" href="<?= BASE_URL ?>/admin/assistant_schedule"
                       class="flex items-center justify-center w-full py-3 rounded-xl bg-indigo-600 text-white font-bold text-xs uppercase tracking-wider hover:bg-indigo-700 shadow-md transition-all transform hover:scale-[1.02]">
        ...
        <div class="w-full md:w-2/3 flex flex-col bg-white relative">
            <div class="flex border-b border-gray-100 px-6 py-4 justify-between items-center shrink-0">
                <h3 class="text-lg font-bold text-gray-800 flex items-center gap-2">
                    <i class="fas fa-chart-pie text-indigo-500"></i> <span id="m_stats_title">Statistik & Kehadiran</span>
                </h3>
            ...
            <!-- [BARU – Tahap 30] Overlay akun nonaktif – ditampilkan via JS
                 saat user.status_account === 'INACTIVE'. Menyensor
                 seluruh panel statistik. -->
...
<!-- [BARU – Tahap 30] Modal Konfirmasi Reset Individu Asisten -->
    ...
    <div class="relative bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden z-10">
        <div class="bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-5 flex items-center gap-3">
            ...
            <div class="text-white">
                <h3 class="font-extrabold text-lg leading-tight">Reset Presensi Asisten</h3>
                <p class="text-orange-100 text-[11px]">Hanya data asisten yang dipilih</p>
            </div>
        </div>
        <div class="p-6 space-y-4">
            <p class="text-sm text-gray-600">Anda akan mereset seluruh data presensi dan logbook milik <span id="singleResetName" class="font-bold text-gray-800"></span>. Data akan diarsipkan dalam ZIP sebelum dihapus.</p>
            <div class="bg-orange-50 border border-orange-200 rounded-2xl p-4 text-xs text-orange-700">
                <p class="font-bold mb-1"><i class="fas fa-info-circle mr-1"></i>Jadwal tidak akan terpengaruh.</p>
                <p>Sebuah file ZIP berisi rekap presensi, logbook, izin, dan jadwal asisten ini akan diunduh otomatis.</p>
            </div>
            <div class="flex gap-3">
                ...
                <button onclick="executeSingleReset()"
                        class="flex-1 py-3 rounded-xl bg-orange-500 text-white font-bold text-sm hover:bg-orange-600 transition">
                    <i class="fas fa-rotate-right mr-2"></i>Ya, Reset Data Ini
                </button>
            </div>
        </div>
    </div>
...
<!-- [BARU – Tahap 30] Modal Konfirmasi Nonaktifkan Akun (from detail) -->
<div id="deactivateModal" class="hidden fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden z-10">
        <div class="bg-gradient-to-r from-gray-700 to-gray-600 px-6 py-5 text-white">
            <h3 class="font-extrabold">Nonaktifkan Akun?</h3>
            <p class="text-gray-300 text-xs mt-1">Data presensi akan diarsipkan + dihapus</p>
        </div>
        <div class="p-6">
            <p class="text-sm text-gray-600 mb-4">Asisten: <span id="deactivateName" class="font-bold text-gray-800"></span></p>
            <div class="flex gap-3">
                <button onclick="closeDeactivateModal()" class="flex-1 py-3 rounded-xl bg-gray-100 text-gray-600 font-bold text-sm">Batal</button>
                <button onclick="executeToggleStatus('INACTIVE')" class="flex-1 py-3 rounded-xl bg-gray-700 text-white font-bold text-sm hover:bg-gray-800">Nonaktifkan</button>
            </div>
        </div>
    </div>
</div>

<!-- [BARU – Tahap 30] Modal Konfirmasi Aktifkan Kembali -->
<div id="reactivateModal" class="hidden fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden z-10">
        <div class="bg-gradient-to-r from-blue-700 to-cyan-600 circuit-pattern relative overflow-hidden px-6 py-5 text-white">
            <h3 class="font-extrabold">Aktifkan Kembali?</h3>
            <p class="text-blue-100 text-xs mt-1">Data presensi lama akan diarsipkan + direset</p>
        </div>
        <div class="p-6">
            <p class="text-sm text-gray-600 mb-4">Asisten: <span id="reactivateName" class="font-bold text-gray-800"></span></p>
            <p class="text-xs text-blue-600 bg-blue-50 border border-blue-100 rounded-xl p-3 mb-4"><i class="fas fa-download mr-1"></i>Sebuah ZIP berisi data sebelum nonaktif akan diunduh otomatis.</p>
            <div class="flex gap-3">
                <button onclick="closeReactivateModal()" class="flex-1 py-3 rounded-xl bg-gray-100 text-gray-600 font-bold text-sm">Batal</button>
                <button onclick="executeToggleStatus('ACTIVE')" class="flex-1 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold text-sm">Aktifkan</button>
            </div>
        </div>
    </div>
</div>

<!-- [BARU – Tahap 30] Modal "Cari Asisten" -->
...
<?php /* [BARU – Tahap 34] Fungsi reset & toggle didefinisikan sebagai
    inline script dengan BASE_URL langsung dari PHP, menghindari:
    (a) masalah scope IIFE di dashboard.js, dan
    (b) mismatch key APP_CONFIG.BASE_URL vs APP_CONFIG.baseUrl. */ ?>
<script>
(function() {
    'use strict';
    var _BASE = '<?= rtrim(BASE_URL, '/') ?>';

    /* ── Single Reset ── */
    window.openSingleResetModal = function() {
        var u = window._currentDetailUser;
        if (!u) return;
        var n = document.getElementById('singleResetName');
        if (n) n.innerText = u.name || u.nama || '';
        var m = document.getElementById('singleResetModal');
    ...
    window.executeSingleReset = function() {
        var u = window._currentDetailUser;
        if (!u) { console.warn('[ICLABS] _currentDetailUser null'); return; }
    ...
    window.openDeactivateModal = function() {
        var u = window._currentDetailUser;
        if (!u) return;
        var n = document.getElementById('deactivateName');
        if (n) n.innerText = u.name || u.nama || '';
        var m = document.getElementById('deactivateModal');
        if (m) m.classList.remove('hidden');
    };
    ...
    window.openReactivateModal = function() {
        var u = window._currentDetailUser;
        if (!u) return;
        var n = document.getElementById('reactivateName');
        if (n) n.innerText = u.name || u.nama || '';
        var m = document.getElementById('reactivateModal');
        if (m) m.classList.remove('hidden');
    };
    ...
    window.executeToggleStatus = function(newStatus) {
        ...
        var u = window._currentDetailUser;
        if (!u) return;
        var userId = u.id_user || u.id;
        fetch(_BASE + '/admin/toggleUserStatus', {
            method: 'POST',
        ...
        .then(function(data) {
            if (data.status === 'success') {
                ...
                if (data.download_url) {
                    ...
                    f.style.display = 'none'; f.src = data.download_url;
                    document.body.appendChild(f);
                    setTimeout(function() { try { document.body.removeChild(f); } catch(e) {} }, 30000);
                }
        ...
        .catch(function() {
            if (typeof showCustomAlert === 'function')
                showCustomAlert('error', 'Gagal', 'Koneksi terputus.');
        });
    };

    /* Pastikan dashboard.js mengenal _currentDetailUser via window global */
    if (!window._currentDetailUser) window._currentDetailUser = null;
})();
</script>
