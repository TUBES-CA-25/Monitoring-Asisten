<div class="max-w-7xl mx-auto space-y-6 animate-enter pb-10">
    ...
    <div class="flex items-center justify-between mb-2">
        <div class="flex items-center gap-4">
            <a href="<?= BASE_URL ?>/admin/dashboard/" class="w-10 h-10 flex items-center justify-center bg-white rounded-full shadow-sm text-gray-500 hover:text-blue-600 hover:bg-blue-50 transition">
                <i class="fas fa-arrow-left"></i>
            </a>
            <div>
                <h1 class="text-2xl font-extrabold text-gray-800">Daftar Jadwal</h1>
                <p class="text-gray-500 text-sm">
                    Asisten: <span class="font-bold text-blue-600"><?= isset($assistant['name']) ? htmlspecialchars($assistant['name']) : 'Asisten Tidak Ditemukan' ?></span>
                </p>
            </div>
        </div>
        ...
        <div class="w-12 h-12 rounded-full p-1 bg-white shadow-md border border-gray-100">
            <?php 
                $assistantName = isset($assistant['name']) ? $assistant['name'] : 'Unknown';
                ...
                if (!empty($photoName) && file_exists($photoPath)) {
                    ...
                    $avatarUrl = "https://ui-avatars.com/api/?name=" . urlencode($assistantName) . "&background=random&size=500";
                }
            ?>
